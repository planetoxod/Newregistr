...
class ProfileModel {
    ...
    private fun authWithPass(onFinishedListener: OnFinishedListener, user: User?) {
        val authApi = RetrofitApi.getInstance().create(AuthApi::class.java)
        val errorList = checkStrings(user)

        if (user != null && errorList.isEmpty()) {
            val loginBody = user.let { LoginBody(it.id, it.phone) }
...
//                mView?.showToast(ErrorText.LoadingError)
    ...
    fun authWithPass(){
        if (profileViewModel.user != null){
            authWithPass(onFinishedListener, profileViewModel.user)
        }
    }
}